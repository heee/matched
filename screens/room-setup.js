  ...
  primaryButton = el("button", {
    ...
    onClick: async () => {
      ...
      try {
        const layout = LAYOUTS[local.layoutId];
        const live = local.mode === "live";
        // Shared and Race boards can pile up several of the same layout in
        // open rooms/invites at once — prefix with the host's name so
        // they're distinguishable at a glance instead of all reading
        // "Dragon's Nest".
        const title = local.mode === "shared" || local.mode === "race" ? `${ctx.state.currentUser}'s ${layout.name}` : layout.name;
